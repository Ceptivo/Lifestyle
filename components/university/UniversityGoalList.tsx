...
import { updateUniversityGoalStatus, deleteUniversityGoal } from "@/app/actions/university";
...
function GoalRow({ goal }: { goal: UniversityGoal }) {
  ...
  return (
    <Card className="flex items-center gap-3 px-4 py-3.5">
      ...
      <button
        type="button"
        disabled={isPending}
        onClick={() => startTransition(() => updateUniversityGoalStatus(goal.id, STATUS_FLOW[goal.status]))}
      ...
      <form action={deleteUniversityGoal.bind(null, goal.id)}>
...
export function UniversityGoalList({ goals }: { goals: UniversityGoal[] }) {
  if (!goals.length) {
    return <p className="text-center text-sm text-charcoal-soft">No university goals yet. Add your first one.</p>;
  }
