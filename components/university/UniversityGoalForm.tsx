...
import { addUniversityGoal } from "@/app/actions/university";
import { Button } from "@/components/ui/Button";
import { Input } from "@/components/ui/Field";

export function UniversityGoalForm() {
  ...
  return (
    <form
      ...
      action={(formData) => {
        startTransition(async () => {
          await addUniversityGoal(formData);
    ...
    >
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-charcoal">New university goal</p>
        <button type="button" onClick={() => setOpen(false)} aria-label="Close" className="p-1 text-charcoal-soft hover:text-charcoal">
          <X size={18} />
        </button>
      </div>

      <Input name="name" placeholder="Goal (e.g. Distinction average)" required />
