...
import { updateWorkGoalStatus, deleteWorkGoal } from "@/app/actions/work";
...
function GoalRow({ goal }: { goal: WorkGoal }) {
  ...
  return (
    <Card className="flex items-center gap-3 px-4 py-3.5">
      ...
      <button
        type="button"
        disabled={isPending}
        onClick={() => startTransition(() => updateWorkGoalStatus(goal.id, STATUS_FLOW[goal.status]))}
      ...
      <form action={deleteWorkGoal.bind(null, goal.id)}>
...
export function WorkGoalList({ goals }: { goals: WorkGoal[] }) {
  if (!goals.length) {
    return <p className="text-center text-sm text-charcoal-soft">No work goals yet. Add your first one.</p>;
  }
