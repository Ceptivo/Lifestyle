    ...
    return (
      <Card className="px-4 py-3.5">
        ...
        >
          ...
          <Textarea name="content" defaultValue={note.content} rows={4} />
    ...
    <Card className="px-4 py-3.5">
      <div className="flex items-start gap-3">
        ...
        <div className="flex shrink-0 gap-1">
          <button type="button" onClick={() => setEditing(true)} aria-label="Edit note" className="rounded-full p-1.5 text-charcoal-soft hover:bg-cream hover:text-charcoal">
            <Pencil size={14} />
          </button>
...
export function NoteList({ notes }: { notes: Note[] }) {
  if (!notes.length) {
    return <p className="text-center text-sm text-charcoal-soft">No notes yet.</p>;
  }
