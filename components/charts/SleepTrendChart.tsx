...
type Point = { date: string; hours: number; dateFormatted: string; hoursFormatted: string };
...
export function SleepTrendChart({ points }: { points: Point[] }) {
  ...
  const { linePath, scaleX, scaleY } = useMemo(() => {
    ...
    const sy = (v: number) => PAD_TOP + innerH - (v / MAX_HOURS) * innerH;
    const line = points.map((p, i) => `${i === 0 ? "M" : "L"}${sx(i).toFixed(2)},${sy(p.hours).toFixed(2)}`).join(" ");
    return { linePath: line, scaleX: sx, scaleY: sy };
  }, [points]);

  if (points.length < 2) {
    return <p className="text-center text-sm text-charcoal-soft">Log a few nights to see a trend.</p>;
  }
  ...
  const activeY = scaleY(activePoint.hours);
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <path d={linePath} fill="none" stroke="#5b9bff" strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />

        {hoverIndex !== null && (
          <line x1={activeX} x2={activeX} y1={PAD_TOP} y2={PLOT_HEIGHT - PAD_BOTTOM} stroke="rgba(255,255,255,0.25)" strokeWidth={1} />
        )}
        <circle cx={activeX} cy={activeY} r={4} fill="#5b9bff" stroke="#19191c" strokeWidth={2} />
      ...
      >
        ...
        <p className="text-sm font-semibold tabular-nums text-charcoal">{activePoint.hoursFormatted}</p>
      </div>
    </div>
  );
}
